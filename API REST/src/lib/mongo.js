...
class MongoLib {
  ...
  async getByEntity (collection, entity) {
    const db = await this.connect()
    const objQuery = db.collection(collection).find({entity: entity})
    console.log(objQuery)
    return db.collection(collection).find({entity: entity}).toArray()
  }
  /*
